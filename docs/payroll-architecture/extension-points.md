...
| **Full TDS engine** | `apps.compliance` TDS service (`tds_engine`, projection, Form 16 prep) | **Implemented (Sprint 9.4)** |
...
### Statutory rule versioning (Sprint 9.1–9.4)
...
- `compliance.FinancialYearTaxRule` + `TaxSlab` store OLD/NEW regime income-tax slabs, std deduction, 87A rebate, cess, surcharge JSON (FY 2024-25 / 2025-26 seeded).
- `PayrollRun.pf_rule_set` / `esi_rule_set` / `pt_rule_set` / `tds_rule_set` and `PayrollPFResult` / `PayrollESIResult` / `PayrollPTResult` / `PayrollTDSResult` snapshot rules used at calculation time.
...
- Resolve TDS by FY + employee regime (declaration / profile; regime-change deadline **31 July** of the FY) + effective-dated tax rule.
...
- Form 16 preparation (`form16_data.py`) is separate from the TDS calculation engine; reports reconcile from snapshots.
### Related
