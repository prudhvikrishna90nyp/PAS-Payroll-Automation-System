...
Recalculation of unlocked runs replaces previous results inside `transaction.atomic`. The run stores `pf_rule_set` and `esi_rule_set` resolved for the period end date.
...
| `statutory.py` + `apps.compliance` | EPF + ESI engines; PT/TDS stubs | **EPF Implemented (Sprint 9.1)**; **ESI Implemented (Sprint 9.2)**; PT/TDS **Planned (9.3–9.4)** |
...
\[
\text{net} = \sum \text{prorated earnings} - \sum \text{prorated structure deductions} - \text{EE PF} - \text{VPF} - \text{EE ESI} - 0_{\text{PT/TDS stubs}}
\]
