...
| UI | Bootstrap 5 + AdminLTE patterns |
| PDF | WeasyPrint |
| Excel | openpyxl |
...
Every business model inherits from `BaseModel` (via `SoftDeleteModel` where soft delete applies):
...
Shared mixins, validators, and utilities live in `apps.common`.
